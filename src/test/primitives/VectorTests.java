...
public class VectorTests {
	...
	public void normalized() {
		...
		Assert.assertTrue("Vector with magnitude larger than 1 not normalized correctly.",
			DoubleCompare
				.eq(Point.ORIGIN.add(new Vector(1, 2, 3).normalized()).distance(Point.ORIGIN), 1));
		Assert.assertTrue("Vector with magnitude smaller than 1 not normalized correctly.",
			DoubleCompare.eq(
				Point.ORIGIN.add(new Vector(0.1, 0.2, 0.3).normalized()).distance(Point.ORIGIN),
				1));

		// Boundary values test
		Assert.assertTrue("Vector with magnitude equal to 1 not normalized correctly.",
			DoubleCompare.eq(Point.ORIGIN.add(
				new Vector(1.414213562373095, 1.414213562373095, 1.414213562373095).normalized())
